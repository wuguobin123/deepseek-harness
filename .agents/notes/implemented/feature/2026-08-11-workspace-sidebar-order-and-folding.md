...
The grouped browser always renders Local workspaces and Cloud workspaces as independent location headers, including an empty location. Each header persists its browser-local disclosure state and offers an add action only while the composed directory-flow hole is occupied. A location add opens that existing flow directly with `location: local` or `location: cloud`; the top header remains a labelled Workspace region with search, view options, and the global location-choice add menu.
...
**Hide empty location headers.** The first Workspace for a location would then require the less direct global menu, and the navigation structure would change as locations become empty or populated.

**Duplicate directory pickers for the two location headers.** Separate flows would drift in cancellation, error recovery, and Workspace adoption. The shared picker accepts an explicit location instead.
...
- Local and cloud location disclosure remains browser-local presentation state. Their add actions preserve the existing local-directory and cloud-copy operations rather than changing Workspace routing.
