...
分组浏览器始终把本机工作区和云端工作区渲染为独立位置区头，即使某个位置为空也不例外。每个区头都会持久化浏览器本地的展开状态，并且只有在组合的目录流 slot 被占用时才提供添加操作。位置添加会以 `location: local` 或 `location: cloud` 直接打开该现有流程；顶部区头仍是带标签的 Workspace 区域，保留搜索、视图选项与全局位置选择添加菜单。
...
**隐藏空位置区头。** 用户添加该位置的第一个 Workspace 时只能转用路径更长的全局菜单，位置变为空或有内容时导航结构也会变化。

**为两个位置区头复制目录选择器。** 分离流程会在取消、错误恢复与 Workspace 接纳语义上逐渐偏离。共享选择器改为接收明确位置。
...
- 本机与云端位置的展开状态保持为浏览器本地呈现状态。它们的添加操作保留现有本地目录与云端副本操作，不改变 Workspace 路由。
- Host Session 记账继续采用[会话列表浏览与 Workspace 手动排序](2026-07-25-session-list-browsing-and-manual-order.zh.md)确立的手动顺序含义。

## 测试

领域与 Host 测试覆盖持久 Workspace 移动、无操作与无效锚点、重启恢复、完整顺序 RPC 响应、顺序帧以及每条 Host stream 基线只读取一份 Workspace 快照。运行时测试覆盖乐观顺序、帧／响应优先级、重叠拒绝后恢复 Host 已确认顺序、重连基线以及 New Session 目标优先级。UI 测试覆盖位置展开与定向位置添加流程、五行折叠、临时展开重置、Workspace 移除后清理持久状态、保持顺序的模式切换、一次性最近更新与“新会话”置顶、首条提示词落地后保留手动拖拽、浏览器本地 Ungrouped 与单列表拖拽持久化、无层级单列表行左侧间距、当前视图标记、展开区段的 Workspace 命中、未裁切的第一条插入边界、列表外 Workspace 与 Session 松手、搜索收起规则和紧凑 CSS 尺寸。
