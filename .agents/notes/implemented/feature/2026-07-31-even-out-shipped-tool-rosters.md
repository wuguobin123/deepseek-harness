...
**`dsh-web-fetch-http`** stays unmounted and `dsh-tool-web` keeps `fetch: false` in the generic shipped roster. That roster is deliberately search-only; product bundles opt into explicit fetch together with their provider selection. The HTTP provider now rejects non-public DNS answers and pins a validated address, and the Xiaowei bundle mounts it without changing the generic preset.
