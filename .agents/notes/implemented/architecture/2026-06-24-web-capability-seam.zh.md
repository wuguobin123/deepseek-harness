...
`web_fetch` 的实现是匿名公网 HTTP(S) fetch 提供方 `http`。它从具体 URL 获取字节，在每次请求前解析目标，只要解析结果中存在非公网地址就拒绝请求，把已验证地址固定到每次连接尝试，并在同一个截止期限下的有界传输重试间轮换这些地址。随后它执行下述传输控制，解码文本内容，并仅返回最小的模型可用结果：最终 URL、状态码、正文和截断标志。它不携带浏览器 cookie、编辑器凭证、git 凭证、内部认证令牌，也不隐式访问私有服务。公开 GitHub 仓库根页面使用 GitHub 匿名 README API，GitHub 与 `raw.githubusercontent.com` 文件 URL 则使用匿名 Contents API 和 raw 响应媒体类型。这使无法连接 GitHub HTML 或 raw 前端的部署仍可读取公开代码文档；结果保留调用方提交的 URL，官方内容端点继续执行相同的目标地址与资源检查。
...
- 每次请求都解析主机名；完整结果中只要有任何 IPv4、IPv6 或 IPv4 映射地址不是公网地址，就拒绝请求；连接固定到一个已验证地址。每个被跟随的重定向都在网络访问前重新解析并验证。
- 请求携带显式的产品 User-Agent，而非静默伪装浏览器。

运行时可以配置一个不同于主 provider 的 fetch 回退 provider。只有主 provider 安全返回 HTTP 403 或 429 时才会调用回退。主路径的安全、传输、超时、取消、重定向、大小或内容表示错误绝不进入回退。回退缺少凭据时保留主响应；其他回退故障保持可见。
...
**Fetch 是网络边界，不仅仅是只读工具。** `web_fetch` 仍可能通过模型选择的 URL 外泄数据，因此它不携带环境凭据，并且只连接到自行解析、验证且固定的公网地址。这能保护内部服务，但不会让不可信网页正文或出站 URL 选择自动成为可信输入。
