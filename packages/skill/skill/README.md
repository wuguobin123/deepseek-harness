...
- `ctx.skills.snapshot({ cwd?, ownerId?, signal?, scope? })` Returns the invocation-neutral `{ skills, complete }` observation for the viewing scope's merged layers. `ownerId` is part of the cache key and lets providers select an account-owned source set. `complete` is false when any provider rejects or explicitly reports incomplete discovery, or when a second catalog revision races the bounded retry; candidates supplied by that observation remain in this result, which is never cached.
- `ctx.skills.list({ cwd?, ownerId?, signal?, scope? })` Borrows the readonly view options, then returns every winning summary for the current workspace and owner, merged across the global layer and the viewing scope's chain and sorted by name. Consumers apply `isModelInvocable(skill)` or `isUserInvocable(skill)` at their own boundary.
- `ctx.skills.get(name, { cwd?, ownerId?, signal?, scope? })` Uses the same readonly options and winning candidate for discovery and loading, rechecks cancellation after discovery or a cache hit, races provider loading against the signal, validates the loaded definition, then returns it regardless of invocation policy.
- `ctx.skills.refresh()` clears completed catalog caches and emits `skills/change`; trusted writers use it after publishing a Skill that must be visible to the next lookup step.
