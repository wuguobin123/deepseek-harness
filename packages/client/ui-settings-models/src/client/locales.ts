...
export const en = {
  ...
  remoteIntro: 'Manage your private API-compatible models for this account.',
  accountEmpty: 'No custom models have been added to this account.',
  accountAdd: 'Add custom model',
  accountKeyHint: 'The API key is encrypted on the server and is never shown again.',
  accountDeleteTitle: 'Delete {model}?',
  accountDeleteDescription: 'Deleting {model} removes it from this account and prevents new requests from using its stored API key.',
  accountDeleteConfirm: 'Delete model',
  accountDeleting: 'Deleting model…',
  accountDeleteFailed: 'The model was already removed or is not owned by this account.',
  edit: 'Edit',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  remoteIntro: '管理当前账户的私有兼容 API 模型。',
  accountEmpty: '当前账户还没有添加自定义模型。',
  accountAdd: '添加自定义模型',
  accountKeyHint: 'API 密钥会在服务端加密保存，保存后不会再次显示。',
  accountDeleteTitle: '删除 {model}？',
  accountDeleteDescription: '删除 {model} 后，它会从当前账户移除，后续请求不能再使用其已保存的 API 密钥。',
  accountDeleteConfirm: '删除模型',
  accountDeleting: '正在删除模型…',
  accountDeleteFailed: '该模型已被删除，或不属于当前账户。',
  edit: '编辑',
  ...
  create: '创建提供方',
  creating: '创建中\u2026',
  onboardingTitle: '添加一个 API 密钥开始使用',
