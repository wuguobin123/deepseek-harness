...
describe('ui-settings-models apply', () => {
  ...
  it('registers account custom models but omits Host configuration and onboarding remotely', async () => {
    const customModels = { list: vi.fn(), create: vi.fn(), remove: vi.fn() }
    const b = await bench(false, undefined, { customModels })
    declare(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const sections = b.slots.entries('settings.section')
    expect(sections).toHaveLength(1)
    expect(sections[0]?.component).toBe(AccountModelsSection)
    const injected = sections[0]?.inject as unknown as () => import('../src/client/AccountModelsSection.tsx').AccountModelsInjected
    expect(injected().api.customModels).toBe(customModels)
