...
分组导航始终分别显示**本机工作区**和**云端工作区**区头。两个标题文字与顶部“工作区”标题保持相同的左侧基线，展开控件紧随标题，添加操作保持在最右侧。每个位置都会记住浏览器本地的展开状态；目录流程存在时，区头尾部会提供添加操作，并以对应的 `location` 直接打开现有流程，即使该分组为空或已收起也不例外。带标签的 Workspace 区域会在顶部区头保留搜索、视图选项和全局添加菜单，因此用户既可以一步添加到已知位置，也可以从完整的位置菜单中选择。
