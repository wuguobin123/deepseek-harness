...
For `workspace-write`, each wrap also returns runner-owned `XDG_CACHE_HOME` and `NPM_CONFIG_CACHE` overrides that point into the temporary area the same profile already authorizes. Bash and PowerShell sandbox executors merge these values after caller environment entries. Read-only wraps return no cache overrides, and `danger-full-access` bypasses this provider. This keeps framework caches usable without granting a user's home cache; the [Xiaowei local runtime reliability decision](../../../.agents/notes/implemented/bug-fix/2026-08-27-xiaowei-local-runtime-interaction-reliability.md) records the failure and alternatives.
