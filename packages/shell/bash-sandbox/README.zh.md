...
- **Runner 环境优先。** confinement 返回环境覆盖项时，前台与后台路径都会在调用方值之后合并这些覆盖项，再启动进程。本地提供方只在 `workspace-write` 下使用该机制，把 `XDG_CACHE_HOME` 与 `NPM_CONFIG_CACHE` 指向已经授权的临时区域；只读与 `danger-full-access` 环境保持不变。
