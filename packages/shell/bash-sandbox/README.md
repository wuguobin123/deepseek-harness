...
- **Runner environment wins.** When confinement returns environment overrides, foreground and background paths merge them after caller values before spawning. The local provider uses this only under `workspace-write` to direct `XDG_CACHE_HOME` and `NPM_CONFIG_CACHE` into its already-authorized temporary area; read-only and `danger-full-access` environments remain unchanged.
