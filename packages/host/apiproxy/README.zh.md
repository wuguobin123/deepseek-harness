...
账户主体会用账户 `userId` 作为 `ownerId` 标记每个新建 Session。Session 列表、搜索、读取、变更、导出、子智能体与目标操作，以及 mux 和 host 事件投递都会在服务端校验该所有者；不存在和属于其他账户的 Session 返回相同的 `session-not-found`。派生会话继承所有者。账户请求绝不会认领 v1 之前未标记所有者的 Session，本地进程内调用方则保留非账户组合所需的既有单用户访问能力。完整决策与迁移影响记录在[账户会话隔离说明](../../../.agents/notes/implemented/bug-fix/2026-08-24-xiaowei-account-session-isolation.zh.md)中。
...
`account.customModels.create/list/remove` 是远程模型设置的账户认证协议。请求不接受用户 ID；handler 从 account principal 派生所属关系，仅返回不含已保存密钥的元数据，并拒绝本地或匿名 principal。create 请求在抵达 `subscribeEnvelopes()` 观察者之前会把密钥替换为 `[redacted]`，但载体仍会发送未修改的真实信封。`session.models` 只会把所寻址 Session 的 owner 的有效记录追加到固定 `xiaowei-custom` 路由下；`session.selectModel` 与 `session.prompt` 会再次校验 owner 与撤销状态，自定义选择也不会写成 Host 全局默认。

`account.plugins.list/install/uninstall` 是插件工厂的账户认证协议。请求只携带部署方拥有的 `pluginId`；handler 从 `request.principal.userId` 派生账户，拒绝本地或匿名 principal，且不接受 activation id、模块名、文件系统路径、配置正文或用户 ID。系统默认目录项始终启用且不可卸载。新账号 Session 会记录可选项选择；安装变化只影响后续 Session，冷恢复与 fork 则从已记录事件组装。
