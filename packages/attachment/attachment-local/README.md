...
Document admission accepts `maxDocumentsPerMessage` files and `maxMessageDocumentBytes` aggregate bytes, with `maxDocumentBytes` applied to each PDF, DOCX, XLSX, or PPTX file. The defaults are 8 files, 128MiB aggregate, and 32MiB per file. The provider validates each file through `@deepseek-ai/dsh-document`, publishes the original bytes under the same immutable object root, and stores a bounded server-generated summary in the reference.
