...
`admitEncodedImages(attachments, images)` is the shared wire entry used by every RPC endpoint that accepts browser image uploads: it enforces canonical base64 on every member, then delegates batch admission — limits, validation, ordered commit — to `saveImages`. `admitEncodedDocuments` applies the same durable-reference rule to PDF, DOCX, XLSX, and PPTX uploads and delegates deployment-configured count, per-file, aggregate-byte, and media-type limits to `saveDocuments`. The local provider validates document bytes with `@deepseek-ai/dsh-document`, stores the original immutable object, and records only its SHA-256 reference and bounded server-generated summary in the Session.
...
- Audio, video, arbitrary files, and persistent unsent drafts require separate lifecycle and provider contracts.
