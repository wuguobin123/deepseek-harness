...
`baseURL` sets the endpoint of every model on the route, while `baseURLEnv` names an endpoint in the immutable launch-environment snapshot; private proxies such as `https://proxy.example.com:8443` remain supported either way. An explicit `baseURL` wins when both fields are present. A named environment value that is missing or empty fails profile resolution instead of silently falling back, and a catalog route that names neither field keeps each catalog model's own endpoint. Naming `api` on a catalog route repoints the whole route at that protocol, which is how a deployment moves a provider between, say, Responses and Chat Completions.
