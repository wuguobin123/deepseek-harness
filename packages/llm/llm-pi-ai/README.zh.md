...
`baseURL` 设定该路由下每个模型的 endpoint，而 `baseURLEnv` 从不可变启动环境快照中点名 endpoint；两种方式都支持 `https://proxy.example.com:8443` 等私有 proxy。两个字段同时存在时，显式 `baseURL` 优先。点名的环境值缺失或为空会让 profile 解析失败，而不是静默降级；两个字段都不写的 catalog 路由会保留每个 catalog 模型自己的 endpoint。在 catalog 路由上点名 `api` 会把整条路由改指到该协议，这正是部署把某个提供方在 Responses 与 Chat Completions 之间迁移的方式。
